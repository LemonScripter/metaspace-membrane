...
| **O-16** | **Antigravity CLI (`agy`) — hook loads but does not fire.** Resolved by experiment, in four steps and without a single guess: (1) **config path** — six candidates seeded with distinct markers, agy's own counter went `0 → 2 hooks.json file(s)`, naming `~/.gemini/hooks.json` and `~/.gemini/antigravity-cli/hooks.json`; (2) **schema shape** — its unmarshal error (`cannot unmarshal string into ... jsonhook.JSONHookSpec`) showed the top level is a flat *name → spec* map, which is what "named hooks" in its log means; (3) **field names** — harvested from the binary's struct tags (`type`, `command`, `prompt`, `model`, `enabled`, `timeout`, `tools`, `events`, `description`, `name`); (4) **validation** — its own validator accepted the reconstructed spec (`loaded 1 named hooks from 1 hooks.json file(s)`). **But it never executed:** a write outside scope went through completely unmediated, with no audit entry. **Loading is not firing, and firing is not blocking — three separate things, each needing its own measurement.** Next hypothesis, from the binary's type names (`jsonhook.preToolPromptResponse`, `postToolPromptResponse`, `stopPromptResponse`): the native event value is likely `preTool`, not `PreToolUse` — the latter probably belongs to its `agy plugin import claude` path. | **empirical run** | OPEN | an Antigravity claim |
