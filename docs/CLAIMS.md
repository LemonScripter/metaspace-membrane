...
| **O-26** | **On Windows an explicit interpreter path is refused: `python.exe` is not `python`.** The allowlist compares `os.path.basename(argv[0])`, so `C:/…/Scripts/python.exe` yields `python.exe`, which no sane constitution lists — while `python` is listed and means the same program. Measured live 2026-07-28 while verifying the 0.3.3 release: running a venv's interpreter by path was refused. It had also appeared twice in the earlier dry-run audit. This is over-blocking, so the direction is safe, but it is reachable in entirely ordinary use — any venv, any absolute interpreter path — and the pressure it creates is to hand-add `.exe` spellings, which doubles every entry and invites someone to add the wrong one. The fix is to strip a trailing `.exe` when comparing on Windows; `.bat`/`.cmd` deliberately NOT included, since a script of that name is not the same artefact as the bare name. Recorded rather than fixed on the day of a release: 0.3.3 had just shipped and an unforced parser change is exactly what a freshly released package should not receive. | **empirical run** (blocked live) + code | OPEN | — |
