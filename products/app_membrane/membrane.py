...
class WasmMembrane:
    ...
    def make_linker(self):
        """The membrane defines the GRANTED gates by NAME (capability import). Anything not
        put here yields 'unknown import' for the guest -> no ambient authority. This is the
        core of the capability membrane."""
        i32x4 = wasmtime.FuncType([wasmtime.ValType.i32()] * 4, [wasmtime.ValType.i32()])
        linker = wasmtime.Linker(self.engine)
        linker.define(self.store, "env", "host_fs_write",
                      wasmtime.Func(self.store, i32x4, self._host_fs_write))
        linker.define(self.store, "env", "host_net_out",
                      wasmtime.Func(self.store, i32x4, self._host_net_out))
        return linker
